...
public:
    ...
    // Constructor with M = # of meanings, S = # of symbols and memory = memory size. Use memory=0 for infinite memory(i.e. no memory).
    // init controls the rowsum of the initial state. If memory!=0 and init>memory, the initial rowsum is equal to memory.
    Agent(int M, int S, int memory, int init);
    ~Agent();

    // Choose a meaning and a symbol
    std::pair<int,int> speak();
    // Choose a meaning for a symbol
    int listen(int sigma);

    int getA(int row, int col);
    // Straight access to association matrix cell. It handles correcting rowsums and colsums vector values
    void setA(int row, int col, int val);
    bool hasMemory();

    //Easy access to association matrix A. These also handle rowsums and colsums values.
    ...
    // and edit the updateHistory queue to hold the correct symbols
